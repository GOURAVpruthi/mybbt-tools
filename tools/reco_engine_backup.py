...
class GSTRecoEngine:
    ...
    def _clean(self,df):
        if "Original Invoice Number" not in df.columns and "Invoice Number" in df.columns:
            df.insert(0, "Original Invoice Number", df["Invoice Number"])
    ...
    def run(self,pr_file,gstr2b_file,output_file,client_name="",email_cfg=None,on_progress=None):
        f=self.feat
        self.on_progress = on_progress
        _step_start = getattr(self, '_step_start', None)
        def prog(pct, msg):
            now = time.time()
            if hasattr(self, '_step_start') and self._step_start and ("Step" in msg or "Excel" in msg):
                elapsed = now - self._step_start
                if elapsed > 0.1:
                    self.log(f"  ⏳ [Previous step took {elapsed:.2f}s]")
            if "Step" in msg or "Excel" in msg or "Loading" in msg or "Cleaning" in msg:
                self._step_start = now
            self.log(msg)
            if self.on_progress: self.on_progress(msg, pct)
            
        self.client_name = (client_name or "").strip()
        prog(5, "═"*56+"\n  GST RECONCILIATION PRO  v5.5\n"+"═"*56)
        if self.client_name:
            self.log(f"  Client: {self.client_name}")
            
        prog(10, "📂 Loading files…")
        
        pr = pd.DataFrame()
        if pr_file:
            pr=self._cc(pd.read_excel(pr_file, header=2))
            pr["Source"]="PR"; pr["UID"]="PR_"+pr.index.astype(str)
            
        b2 = pd.DataFrame()
        if gstr2b_file:
            b2=self._cc(pd.read_excel(gstr2b_file, header=2))
            b2["Source"]="GSTR2B"; b2["UID"]="2B_"+b2.index.astype(str)
        ...
        if f.get("user_knockoff",True):
            if not pr.empty: pr=pr[~pr["UID"].isin(ko_pr["UID"])]
            if not b2.empty: b2=b2[~b2["UID"].isin(ko_b2["UID"])]
            self.log(f"  User knockoffs → PR:{len(ko_pr)}  2B:{len(ko_b2)}")
            
        self.log(f"  PR={len(pr)} | 2B={len(b2)}")
        prog(15, "🧹 Cleaning & deriving columns…")
        if not pr.empty: pr=self._clean(pr)
        if not b2.empty: b2=self._clean(b2)
        ...
        pr_ko=pd.DataFrame()
        if not pr.empty and f.get("pr_knockout",True) and self.pr_ko:
            prog(20, "🔄 Step 1 — PR Knockout")
            pr_ko,pr=self._knockout(pr,self.pr_ko,"PR",50000); t_pr+=len(pr_ko)
        else: self.log("\n⏭ Step 1 — PR Knockout DISABLED or SKIP")

        grp=pd.DataFrame()
        if not single_file_mode and f.get("grouped_match",True):
            prog(30, "🔄 Step 2 — Exact Group Match")
            grp,pr,b2=self._grouped(pr,b2)
        else: self.log("\n⏭ Step 2 — Grouped DISABLED or SKIP")

        rule=pd.DataFrame()
        if not single_file_mode and f.get("rule_match",True):
            prog(40, "🔄 Step 3 — Rule Match")
            rule,pr,b2=self._rules(pr,b2)
        else: self.log("\n⏭ Step 3 — Rule Match DISABLED or SKIP")

        fuz=pd.DataFrame()
        if not single_file_mode and f.get("fuzzy_invoice",True):
            prog(50, "🔄 Step 4 — Fuzzy Invoice Match")
            fuz,pr,b2=self._fuzzy(pr,b2)
        else: self.log("\n⏭ Step 4 — Fuzzy DISABLED or SKIP")

        vnd=pd.DataFrame()
        if not single_file_mode and f.get("vendor_fuzzy",False):
            prog(60, "🔄 Step 5 — Vendor Name Fuzzy")
            vnd,pr,b2=self._vendor_fuzzy(pr,b2)
        else: self.log("\n⏭ Step 5 — Vendor Fuzzy DISABLED or SKIP")

        bnk=pd.DataFrame()
        if not single_file_mode and f.get("bank_match",False):
            prog(70, "🔄 Step 6 — Bank-Entry Match (Max Match)")
            bnk,pr,b2=self._bank_match(pr,b2)
        else:
            self.log("\n⏭ Step 6 — Bank Match DISABLED or SKIP")

        b2_ko=pd.DataFrame()
        if not b2.empty and f.get("2b_knockout",True) and self.b2_ko:
            prog(80, "🔄 Step 7 — 2B Knockout")
        ...
        def extract_uids(df, uid_col):
            ...
            s = df[[uid_col, 'Match_Remark']].copy() if 'Match_Remark' in df.columns else df[[uid_col]].copy()
            if 'Match_Remark' not in s.columns: s['Match_Remark'] = 'Matched'
            s[uid_col] = s[uid_col].astype(str).str.split(',')
            s = s.explode(uid_col)
            s[uid_col] = s[uid_col].str.strip()
            s = s[s[uid_col] != '']
            s = s.drop_duplicates(uid_col, keep='first')
        ...
        for df in [grp, rule, fuz, vnd, bnk, pr_ko, b2_ko, ko_pr, ko_b2, pr, b2, unm]:
            ...
            if 'UID' in df.columns: all_uid_series.append(extract_uids(df, 'UID'))
            
        uid_to_remark = {}
        if all_uid_series:
            uid_to_remark = pd.concat(all_uid_series).to_dict()

        if hasattr(self, 'pr_raw') and not self.pr_raw.empty:
            self.pr_raw.insert(0, "Reco_Remark", self.pr_raw["UID"].map(uid_to_remark).fillna("Unknown or Leftover"))
            self.pr_raw.drop(columns=["UID", "Source"], inplace=True, errors="ignore")
    ...
    def _generate_analysis(self, pr, b2, all_m, pr_ko, b2_ko, unm):
        self.log("\n🔄 Generating Remarks-Wise Analysis Pivot")
        rows = []
        if not all_m.empty:
            for _, r in all_m.iterrows():
                rows.append({
                    "Match_Remark": r.get("Match_Remark", "Matched (No Remark)"),
                    "Party": r.get("Vendor Name_PR") or r.get("Vendor Name_2B") or "Unknown",
                    "Count": 1,
                    "PR_Amount": float(r.get("Total Tax_PR", 0) or 0),
                    "2B_Amount": float(r.get("Total Tax_2B", 0) or 0),
                    "Variance": float(r.get("Total Tax_PR", 0) or 0) - float(r.get("Total Tax_2B", 0) or 0)
                })
        if not unm.empty:
            for _, r in unm.iterrows():
                party = r.get("Vendor Name") or r.get("Vendor Name_PR") or r.get("Vendor Name_2B") or "Unknown"
                pr_amt = float(r.get("Total Tax", 0)) if r.get("Match_Status") == "Only_in_PR" else 0
                b2_amt = float(r.get("Total Tax", 0)) if r.get("Match_Status") == "Only_in_2B" else 0
                rows.append({
                    "Match_Remark": r.get("Match_Remark", r.get("Match_Status", "Unmatched")),
                    "Party": party,
                    "Count": 1,
                    "PR_Amount": pr_amt,
                    "2B_Amount": b2_amt,
                    "Variance": pr_amt - b2_amt
                })
        
        if not rows: return pd.DataFrame()
        
        df = pd.DataFrame(rows)
        pivot = df.groupby(["Match_Remark", "Party"]).agg({
            "Count": "sum", "PR_Amount": "sum", "2B_Amount": "sum", "Variance": "sum"
        }).reset_index()
        
        pivot = pivot.sort_values(by=["Match_Remark", "Variance"], ascending=[True, False])
        
        totals = pd.DataFrame([{
            "Match_Remark": "GRAND TOTAL",
            "Party": "",
            "Count": pivot["Count"].sum(),
            "PR_Amount": pivot["PR_Amount"].sum(),
            "2B_Amount": pivot["2B_Amount"].sum(),
            "Variance": pivot["Variance"].sum()
        }])
        
        return pd.concat([pivot, totals], ignore_index=True)
        
    def _reorder_matched_cols(self, df):
        if df.empty: return df
        pr_cols = [c for c in df.columns if str(c).endswith("_PR")]
        b2_cols = [c for c in df.columns if str(c).endswith("_2B")]
        other = [c for c in df.columns if c not in pr_cols and c not in b2_cols and c != "|--- VS ---|"]
        
        front = []
        for c in ["Match Status", "Match_Remark", "Matched On", "Check_Reason"]:
            if c in other:
                front.append(c)
                other.remove(c)
        
        df["|--- VS ---|"] = "<-->"
        ordered = front + pr_cols + ["|--- VS ---|"] + b2_cols + other
        return df[ordered]
...
def _fmt_sheet(ws, status_col=None, client_name=""):
    ...
    HF_DEF=PatternFill("solid",fgColor="4F46E5") # Blue default
    HF_PR=PatternFill("solid",fgColor="0EA5E9")  # Light Blue PR
    HF_2B=PatternFill("solid",fgColor="10B981")  # Emerald 2B
    HF_VS=PatternFill("solid",fgColor="F59E0B")  # Amber VS
    ...
    for cell in ws[header_row]:
        ...
        fill = HF_DEF
        if val.endswith("_PR"): fill = HF_PR
        elif val.endswith("_2B"): fill = HF_2B
        elif val == "|--- VS ---|": fill = HF_VS
        cell.fill=fill; cell.font=HFT; cell.alignment=HA; cell.border=BDR
        
    ws.row_dimensions[header_row].height=28
    ws.freeze_panes=ws.cell(row=header_row+1,column=1).coordinate
    
    # Removed slow iter_rows for data cells to fix 4 minute save delay
...
def _append_footer(ws):
    ...
    ws.cell(row=foot_row, column=1, value="✨  Made by BBT  ✨")
